  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${APP_URL}/api/auth/callback?next=${encodeURIComponent(next)}`,
        },
      })
      if (error) {
    ...
    <main className="min-h-screen flex items-center justify-center p-4 bg-bg-surface">
      ...
      >
        ...
        >
          ...
          <p className="text-sm text-text-secondary mb-7">Sign in to continue</p>
          ...
          <p className="text-center text-sm text-text-secondary mt-6">
            ...
            <Link href="/register" className="text-accent-primary hover:text-accent-glow transition-colors font-medium">
              Register →
            </Link>
          </p>
        </motion.div>
